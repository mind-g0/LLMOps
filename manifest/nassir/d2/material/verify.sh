...
fail() {
    ...
    kubectl delete pod "$PROBER" -n nassir --ignore-not-found >/dev/null 2>&1
    exit 1
}

command -v kubectl >/dev/null || fail "kubectl not on PATH"
kubectl get deployment "$DEPLOY" -n nassir >/dev/null 2>&1 || fail "no deployment '$DEPLOY'"
kubectl get service "$SVC" -n nassir >/dev/null 2>&1 || fail "no service '$SVC'"

ready=$(kubectl get deployment "$DEPLOY" -n nassir -o jsonpath='{.status.readyReplicas}')
[ "${ready:-0}" -ge 2 ] || fail "deployment has ${ready:-0} ready replicas, need 2"

spec=$(kubectl get deployment "$DEPLOY" -o json -n nassir)
...
maxunavail=$(kubectl get deployment "$DEPLOY" -n nassir -o jsonpath='{.spec.strategy.rollingUpdate.maxUnavailable}')
[ "$maxunavail" = "0" ] || fail "maxUnavailable is '${maxunavail:-<default>}', the zero-downtime bar needs 0"

image=$(kubectl get deployment "$DEPLOY" -n nassir -o jsonpath='{.spec.template.spec.containers[0].image}')
case "$image" in
    *"<your-user>"*) fail "deployment still carries the placeholder image" ;;
esac
...
' || fail "could not start the prober pod"

sleep 5

stamp="verify-$(date +%s)"

kubectl set env deployment/"$DEPLOY" VERIFY_STAMP="$stamp" -n nassir >/dev/null \
    || fail "could not trigger a rolling update"

kubectl rollout status deployment/"$DEPLOY" -n nassir --timeout=120s >/dev/null \
...
line=$(kubectl logs "$PROBER" -n nassir | tail -1)

kubectl delete pod "$PROBER" -n nassir >/dev/null 2>&1
...
[ "$okn" -ge 200 ] \
    || fail "prober only landed $okn requests; something throttled the watch"

[ "$badn" -eq 0 ] \
    || fail "$badn requests failed during the rolling update; the zero-downtime bar is zero"
