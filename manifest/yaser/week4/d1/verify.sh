...
ctx=$(kubectl config current-context $your_namespace 2>/dev/null || true)
...
phase=$(kubectl get pod "$POD" -o jsonpath='{.status.phase}' $your_namespace 2>/dev/null) \
...
ready=$(kubectl get pod "$POD" -o jsonpath='{.status.containerStatuses[0].ready}' $your_namespace )
[ "$ready" = "true" ] || fail "pod is Running but not Ready; /health is still 503 (kubectl logs $POD)"

image=$(kubectl get pod "$POD" -o jsonpath='{.spec.containers[0].image}' $your_namespace)
case "$image" in
  *"<your-user>"*) fail "pod.yaml still carries the placeholder image line; edit it to your own image" ;;
esac
echo "pod image: $image"

kubectl port-forward "pod/$POD" "$PORT:8000" $your_namespace >/dev/null 2>&1 &
PF_PID=$!
up=""
...
node=$(kubectl get pod "$POD" -o jsonpath='{.spec.nodeName}' $your_namespace)
