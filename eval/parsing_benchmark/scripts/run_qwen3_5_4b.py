...
from dataset_utils import (iter_dataset_samples, get_run_id, write_run_manifest, build_processor_and_model,
                            load_image_pages, get_decoding, MAX_NEW_TOKENS_DEFAULT)
from prompts import get_prompt
 
BASE = Path(__file__).parent.parent
SAMPLE_ROOT = BASE / "data" / "samples"
PIPELINE_NAME = "qwen3_5_4b"
MODEL_ID = "Qwen/Qwen3.5-4B"
MODEL_REVISION = "main"  # pin an exact commit before the real 200-sample run
...
def run_one_doc(processor, model, prompt: str, doc_path: Path, decoding: dict) -> tuple[str, int]:
    ...
    for page_img in pages:
        ...
        inputs = processor.apply_chat_template(
            ...
            enable_thinking=False,  # skip the <think> reasoning block -- real latency cost, no benefit for OCR
...
def main():
    ...
    processor, model = build_processor_and_model(
        MODEL_ID, MODEL_REVISION, min_pixels=MIN_PIXELS, max_pixels=MAX_PIXELS,
    ...
    for dataset_name, _, docs in iter_dataset_samples(SAMPLE_ROOT):
        ...
        # see prompts.get_prompt(). "control" gives the bare minimal
        ...
        for doc_path in tqdm(docs, desc=f"Qwen3.5-4B [{dataset_name}]"):
        ...
        write_run_manifest(
            ...
            decoding_params={**decoding, "max_new_tokens": MAX_NEW_TOKENS_DEFAULT, "enable_thinking": False},
